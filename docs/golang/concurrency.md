...
1. 并发偏重于多个任务交替执行,而多个任务之间有可能还是串行的.而并行是真正意义上的"同时执行".
...
Channel 是  goroutine 之间的通信方式,可以使用 channel 在两个或多个 goroutine 之间传递消息.Channel也可以理解是一个先进先出的队列,通过管道进行通信.

Go 奉行通过通信来共享内存,而不是共享内存来通信.所以,channel 是goroutine之间互相通信的通道,goroutine之间可以通过它发消息和接收消息.
Go的设计思想就是, 不要通过共享内存来通信,而是通过通信来共享内存,前者就是传统的加锁,后者就是Channel.设计Channel的主要目的就是在多任务间传递数据的,本身就是安全的.
...
Golang 中的有两种锁,为互斥锁 sync.Mutex 和读写锁 sync.RWMutex.
...
### 1.25.1. goroutine的优雅退出

#### 1.25.1.1. 使用for-range退出
...
#### 1.25.1.2. 使用select case ,ok退出

解决是当读入数据的通道关闭时，没数据读时程序的正常结束

##### 1.25.1.2.1. 如果某个通道关闭后，需要退出协程，直接return
...
##### 1.25.1.2.2. 如果某个通道关闭了，不再处理该通道，而是继续处理其他case，退出是等待所有的可读通道关闭。我们需要使用select的一个特征：select不会在nil的通道上进行等待。这种情况，把只读通道设置为nil即可解决
