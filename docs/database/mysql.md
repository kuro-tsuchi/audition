...
MySQL 是一种开源的关系型数据库,主要用于持久化的存储系统中的一些数据,MySQL 的默认端口号是 3306.
...
在一个事务读取一个数据时,另外一个事务也访问了该数据,那么在第一个事务中修改了这个数据后,第二个事务也修改了这个数据.这样第一个事务内的修改结果就被丢失

#### 1.7.4.3. 不可重复读 (Unrepeatable read)

在一个事务内多次读同一数据.在这个事务还没有结束时,另一个事务也访问该数据.那么,在第一个事务中的两次读数据之间,由于第二个事务的修改导致第一个事务两次读取的数据不一致

#### 1.7.4.4. 幻读 (Phantom read)

一个事务读取了几行数据,接着另一个并发事务插入 (或删除) 几条数据.在随后的查询中,第一个事务就会发现多了一些原本不存在的记录
...
## 1.8. 什么是 MySQL binlog

Binlog 是用来记录 Mysql 内部对 数据库的改动（只记录对数据的修改操作），主要用于数据库的主从复制以及增量恢复。
