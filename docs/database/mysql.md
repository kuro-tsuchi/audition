...
MVCC 是指多版本并发控制 (Multi-Version Concurrency Control).MVCC 是在并发访问数据库时，通过对数据进行多版本控制，避免因写锁而导致读操作的堵塞，从而很好的优化并发堵塞问题。

MVCC 通过对数据进行多版本保存，根据比较版本号来控制数据是否展示，从而达到读取数据时无需加锁就可以实现事务的隔离性。
...
在典型的应用程序中，多个事务并发运行，经常会操作相同的数据来完成各自的任务，比如多个用户对同一数据进行操作。
...
![picture 8](../.vuepress/public/assets/images/1640433930508.png)  
