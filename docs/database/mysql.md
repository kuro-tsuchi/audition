...
### 1.2.3. 关于 MyISAM 和 InnoDB 的选择问题
...
### 1.3.1. MyISAM 和 InnoDB 存储引擎使用的锁
...
执行查询语句的时候,会先查询缓存.不过,MySQL 8.0 版本后移除查询缓存,因为这个功能不太实用
...
1. SERIALIZABLE(可串行化): 最高的隔离级别,完全服从 ACID 的隔离级别.所有的事务依次逐个执行,这样事务之间就完全不可能产生干扰,也就是说,该级别可以防止脏读,不可重复读以及幻读.
...
### 1.5.6. MySQL 的默认隔离级别是什么?

MySQL InnoDB 存储引擎的默认支持的隔离级别是 REPEATABLE-READ(可重读).我们可以通过`SELECT @@transaction_isolation;`命令来查看,
