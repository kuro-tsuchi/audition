...
MVCC 是指多版本并发控制 (Multi-Version Concurrency Control). MVCC 是在并发访问数据库时, 通过对数据进行多版本保存, 根据比较版本号来控制数据是否展示, 从而达到读取数据时无需加锁就可以实现事务的隔离性, 避免因写锁而导致读操作的堵塞
...
MySQL InnoDB 存储引擎的默认支持的隔离级别是 REPEATABLE-READ , InnoDB 存储引擎在 分布式事务的情况下一般会用到 SERIALIZABLE 隔离级别.

隔离级别越低, 事务请求的锁越少, 所以大部分数据库系统的隔离级别都是 READ-COMMITTED, InnoDB 存储引擎默认使用 REPEATABLE-READ 并不会有任何性能损失. `SELECT @@transaction_isolation;` 可以查询当前隔离级别
...
索引是一种特殊的文件, 它们包含着对数据表里所有记录的引用指针. InnoDB 数据表上的索引是表空间的一个组成部分
...
字段非空: 应该指定列为 NOT NUL.在 mysql 中, 含有空值的列很难进行查询优化, 因为它们使得索引, 索引的统计信息以及比较运算更加复杂. 你应该用 0, 一个特殊的值或者一个空串代替空值;
