...
## 1.2. 5.Redis和MySQL的区别

1. 从类型上来说,MySQL是关系型数据库,Redis是缓存数据库。
1. MySQL用于持久化的存储数据到硬盘,功能强大,但是速度较慢, Redis用于存储使用较为频繁的数据到缓存中,读取速度快
...
### 1.9.1. 文件事件处理器 (file event handler) 主要是包含 4 个部分
