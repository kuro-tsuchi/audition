...
1. #{}是 sql 的参数占位符,MyBatis 会将 sql 中的#{}替换为? 号,在 sql 执行前会使用 PreparedStatement 的参数设置方法,按序给 sql 的? 号占位符设置参数值,比如 ps.setInt(0, parameterValue),#{item.name} 的取值方式为使用反射从参数对象中获取 item 对象的 name 属性值,相当于 param.getItem().getName().

## 1.2. 2,Xml 映射文件中,除了常见的 select|insert|update|delete 标签之外,还有哪些标签?
...
## 1.3. 3,最佳实践中,通常一个 Xml 映射文件,都会写一个 Dao 接口与之对应,请问,这个 Dao 接口的工作原理是什么?Dao 接口里的方法,参数不同时,方法能重载吗?
...
## 1.5. 4,MyBatis 是如何进行分页的?分页插件的原理是什么?
...
举例: select _from student ,拦截 sql 后重写为: select t._ from (select \* from student)t limit 0,10

## 1.6. 5,简述 MyBatis 的插件运行原理,以及如何编写一个插件
...
## 1.7. 6,MyBatis 执行批量插入,能返回数据库主键列表吗?

注:我出的.

能,JDBC 都能,MyBatis 当然也能.

## 1.8. 7,MyBatis 动态 sql 是做什么的?都有哪些动态 sql?能简述一下动态 sql 的执行原理不?
...
## 1.9. 8,MyBatis 是如何将 sql 执行结果封装为目标对象并返回的?都有哪些映射形式?
...
## 1.10. 9,MyBatis 能执行一对一,一对多的关联查询吗?都有哪些实现方式,以及它们之间的区别
...
## 1.11. 10,MyBatis 是否支持延迟加载?如果支持,它的实现原理是什么?
...
## 1.12. 11,MyBatis 的 Xml 映射文件中,不同的 Xml 映射文件,id 是否可以重复?
...
## 1.13. 12,MyBatis 中如何执行批处理?

注:我出的.

使用 BatchExecutor 完成批处理.

## 1.14. 13,MyBatis 都有哪些 Executor 执行器?它们之间的区别是什么?
...
## 1.15. 14,MyBatis 中如何指定使用哪一种 Executor 执行器?

注:我出的

在 MyBatis 配置文件中,可以指定默认的 ExecutorType 执行器类型,也可以手动给 DefaultSqlSessionFactory 的创建 SqlSession 的方法传递 ExecutorType 类型参数.

## 1.16. 15,MyBatis 是否可以映射 Enum 枚举类?
...
## 1.17. 16,MyBatis 映射文件中,如果 A 标签通过 include 引用了 B 标签的内容,请问,B 标签能否定义在 A 标签的后面,还是说必须定义在 A 标签的前面?
...
## 1.18. 17,简述 MyBatis 的 Xml 映射文件和 MyBatis 内部数据结构之间的映射关系?

注:我出的

MyBatis 将所有 Xml 配置信息都封装到 All-In-One 重量级对象 Configuration 内部.在 Xml 映射文件中, <parameterMap> 标签会被解析为 ParameterMap 对象,其每个子元素会被解析为 ParameterMapping 对象. <resultMap> 标签会被解析为 ResultMap 对象,其每个子元素会被解析为 ResultMapping 对象.每一个 <select>,<insert>,<update>,<delete> 标签均会被解析为 MappedStatement 对象,标签内的 sql 会被解析为 BoundSql 对象.

## 1.19. 18,为什么说 MyBatis 是半自动 ORM 映射工具?它与全自动的区别在哪里?
