...
## 1.10. MyBatis 是否支持延迟加载? 如果支持,它的实现原理是什么?
...
## 1.11. MyBatis 的 Xml 映射文件中,不同的 Xml 映射文件,id 是否可以重复?
...
## 1.13. MyBatis 都有哪些 Executor 执行器?它们之间的区别是什么?
...
## 1.14. MyBatis 中如何指定使用哪一种 Executor 执行器?

在 MyBatis 配置文件中,可以指定默认的 ExecutorType 执行器类型,也可以手动给 DefaultSqlSessionFactory 的创建 SqlSession 的方法传递 ExecutorType 类型参数.

## 1.15. MyBatis 是否可以映射 Enum 枚举类?
...
## 1.16. MyBatis 映射文件中,如果 A 标签通过 include 引用了 B 标签的内容,请问,B标签能否定义在 A 标签的后面,还是说必须定义在 A 标签的前面?
