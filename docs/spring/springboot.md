...
1. 声明式事务 : 在 XML 配置文件中配置或者直接基于注解(推荐使用) : 实际是通过 AOP 实现(基于@Transactional 的全注解方式使用最多)
...
### 1.8.1. 如何使用 JPA 在数据库中非持久化一个字段?

如果想让 secrect 这个字段不被持久化,也就是不被数据库存储,可以采用下面4种方法, 一般使用后面两种方式比较多,个人使用注解的方式比较多.
