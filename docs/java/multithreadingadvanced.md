...
给当前类加锁, 会作用于类的所有对象实例, 进入同步代码前要获得当前 class 的锁. 因为静态成员不属于任何一个实例对象, 是类成员(_static 表明这是该类的一个静态资源, 不管 new 了多少个对象, 只有一份_). 所以, 如果一个线程 A 调用一个实例对象的非静态 synchronized 方法, 而线程 B 需要调用这个实例对象所属类的静态 synchronized 方法, 是允许的, 不会发生互斥现象, 因为访问静态 synchronized 方法占用的锁是当前类的锁, 而访问非静态 synchronized 方法占用的锁是当前实例对象锁.
...
1. synchronized 关键字加到实例方法上是给对象实例上锁.
1. 尽量不要使用 synchronized(Stringa)因为 JVM 中, 字符串常量池具有缓存功能!
...
## 1.8. 说说 synchronized 关键字和 volatile 关键字的区别
...
## 1.12. 实现 Runnable 接口和 Callable 接口的区别
...
## 1.13. 执行 execute()方法和 submit()方法的区别是什么呢?
...
### 1.14.1. 通过 Executor 框架的工具类 Executors 来实现
...
### 1.14.3. threadPoolExecutor 构造函数重要参数分析

#### 1.14.3.1. ThreadPoolExecutor 3 个最重要的参数
...
#### 1.14.3.2. ThreadPoolExecutor 其他常见参数
