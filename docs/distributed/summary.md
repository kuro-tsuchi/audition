...
在分布式系统中,经常需要一些全局唯一的 ID 对数据,消息,http 请求等进行唯一标识. 全局唯一 ID 就是分布式 ID
...
### 1.17.2. 先更新缓存,再更新数据库 (pass)

更新完缓存后,数据库由于某种原因宕机了,直接 pass 掉.

### 1.17.3. 先更新数据库,再更新缓存 (pass)
...
## 1.18. 认证(Authentication)和授权(Authorization)的区别是什么?
...
## 1.19. Cookie 和 Session 有什么区别?如何使用 Session 进行身份验证?
...
## 1.20. 什么是 Token? 什么是 JWT? 如何基于 Token 进行身份验证?
